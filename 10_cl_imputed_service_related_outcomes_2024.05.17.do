...
//merge appointment data, follow-up data with randomisation status and resave ready for analytical file
		foreach i in 3 6 {	
			...
			order pcmis_iaptus_no rand_allocation site 
			compress
			save working_data/imputed_case_service_outcomes_`i'm, replace
			}
						
	
	
	//merge service outcomes files at 3 & 6 months follow-up to create analytical file
		use working_data/imputed_service_outcomes_3m, clear
		merge 1:1 	pcmis_iaptus_no using working_data/imputed_service_outcomes_6m
		drop _m 
		compress
		save working_data/imputed_service_outcomes_analysis, replace
