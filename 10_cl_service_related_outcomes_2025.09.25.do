...
//merge appointment data, follow-up data with randomisation status and resave ready for analytical file
		...
		//assume "9 n/a" data ==.
			...
			//compress, order and save complete case data set
			compress
			order pcmis_iaptus_no rand_allocation site 
			compress
			save working_data/service_outcomes_cc_`i'm, replace
			}
						
	//open and resave save with missing values imputed.
	foreach i in 3 6 {
	use working_data/service_outcomes_cc_`i'm, clear
			 //assume missing data ==0
			foreach var in iapt_completed_treatment_`i'm iapt_discontinue_treatment_`i'm iapt_ongoing_treatment_`i'm {
				replace `var'=0 if `var'==. 
				}
	...
	save working_data/service_outcomes_itt_`i'm, replace
	}
	
	//merge service outcomes files at 3 & 6 months follow-up to create analytical file
		use working_data/service_outcomes_cc_3m, clear
		merge 1:1 pcmis_iaptus_no using working_data/service_outcomes_cc_6m
		drop _m 
		merge 1:1 pcmis_iaptus_no using working_data/service_outcomes_itt_3m
			drop _m
		compress
		merge 1:1 pcmis_iaptus_no using working_data/service_outcomes_itt_6m
		compress
			drop _m

		save working_data/service_outcomes_analysis, replace
